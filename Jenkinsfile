pipeline{
    ...
    stages{
        ...
        stage('Build_Prod'){
            ...
            steps{
                 //验证目录是否存在，不存在则创建
                 sh "sshpass -p ${_deployMap['_targetPwd']} ssh -p ${_deployMap['_targetPort']} ${_deployMap['_targetUser']}@${_deployMap['_targetIP']} \"[ -d ${_buildPath}/${env.JOB_NAME}/ ] && `echo '1 < 2'` || mkdir -p ${_buildPath}/${env.JOB_NAME}/logs\""
                 //远程传输jar包和启动脚本
